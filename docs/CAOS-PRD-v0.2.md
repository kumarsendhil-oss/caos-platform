...
### Prepared for Venture Assist / Srivatsan & Associates | v0.2.8
...
| v0.2.6 | 2026-09-21 | **§3's Sprint Plan row no longer states a week count.** It read "~27 weeks, timeline pending Phase 0 spike confirmation"; both halves went stale on 2026-09-21 when the sprint plan reconfirmed its Sprint 1–2 estimate after P0-06 reported — Foundation became a three-iteration phase and the total moved to ~29 weeks. Rather than update the figure, the row now **points at the sprint plan** and says the timeline was reconfirmed. Same reasoning as v0.2.5 applied to a different field: a number copied from another document is a second place to update and a second place to go stale, and this row had already gone stale once. Conventions §9 — one place states a fact, everything else points. **No sprint was resequenced by this entry**; the sprint plan holds that decision and sprint numbers are unchanged, so every `Sprint N` reference elsewhere in this PRD (including PM-05 at Sprint 11) remains correct. |
| v0.2.7 | 2026-09-21 | **§3 gains a row for the customer-facing Software Development Proposal**, which had no home in this table despite being a customer-facing artifact with no changelog of its own — exactly the case §3 exists for (conventions §9). The row is carried at **v0.3**, the version that introduces the commercial terms: a fixed price of ₹15,00,000 exclusive of GST against milestones M1–M4, replacing the previous ₹24L–₹35.5L phase-wise range; a 120-day post-go-live warranty with defect severity levels; training; and a licence and ownership model, all in a new §10. The row also records that the proposal is **blocked from being sent** until its §9 recurring-cost estimate is re-priced off Supabase/Railway onto the ADR 0007/0009 AWS ap-south-1 stack (`PENDING:038`) — a defect that predates v0.3 and is not introduced by it. A version bump and row rather than an erratum, per §8: a reader of §3 now learns that an artifact exists which the table did not previously list, and that it is not yet sendable. |
| v0.2.8 | 2026-09-21 | **§3's Software Development Proposal row moves to v0.4, and the not-yet-sendable note is removed.** v0.3 priced §9's recurring operating costs on Supabase and Railway and its OCR line on AWS Textract, contradicting ADR 0007 (AWS ap-south-1), ADR 0009 (single-tenant per-practice deployments), ADR 0014 and — for Textract — ADR 0008's self-hosted PaddleOCR decision. v0.4 re-prices §9 on AWS ap-south-1 against rates read from AWS's published price list on 2026-09-21, replaces the Textract line with self-hosted OCR compute, and states the per-practice fixed floor that ADR 0009 makes recur for every firm. **`PENDING:038` closes with this**, so the block on sending the proposal is lifted. A version bump and row rather than an erratum, per §8: §3 previously told a reader the proposal could not be sent, and that has stopped being true. |
...
| Sprint Plan | Complete — Phase 0 (+ Zoho spike) + 12 numbered sprints. **Timeline reconfirmed 2026-09-21** after P0-06 reported: Foundation is a three-iteration phase and sprint numbers are unchanged. The week count is deliberately not repeated here — the sprint plan states it, and a copy in this table is a second place to go stale | See its changelog |
| Software Development Proposal (customer-facing) | Complete — **v0.4 (2026-09-21)**. Commercial terms: a fixed price of ₹15,00,000 exclusive of GST against milestones M1–M4, a 120-day post-go-live warranty, training, defect severity levels, and the licence and ownership model (§10). §9's recurring operating costs are priced on the **AWS ap-south-1 single-tenant stack** per ADR 0007/0009, with self-hosted OCR per ADR 0008 | v0.4 |
