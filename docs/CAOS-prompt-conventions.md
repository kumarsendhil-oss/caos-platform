...
3. **`python scripts/check_md_tables.py docs/`, `python scripts/check_doc_versions.py docs/`, and `python scripts/check_readme_refs.py docs/`, with their exit codes.** All three, whenever anything under `docs/` changed. `check_md_tables.py` also runs automatically on each docs edit via a PostToolUse hook; `check_doc_versions.py` and `check_readme_refs.py` deliberately do not, because the invariant each asserts only holds once a multi-step edit is finished — header-and-changelog consistency for the one, and for the other, whether a referenced file has actually been committed yet (it reads the git index, so a written-but-unstaged file reads as missing). This block is where both get checked.
