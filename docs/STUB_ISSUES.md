...
| PENDING:010 | `app/books_connector/tally_adapter.py` — post-verification correlation (BK-07) | Carry-forward of finding #10's open item and #11's `REMOTEID`/`VCHKEY` note. #14 rules out `VOUCHERNUMBER` (Tally auto-assigns it). **#15 narrows this to `REMOTEID`, not `VCHKEY`**: `REMOTEID` is byte-identical to the voucher's own `GUID`, stable across repeated reads, and — as of the 2026-09-16 restart test — stable across a full TallyPrime restart. `VCHKEY` stays demoted on structural grounds (opaque constant plus storage offset); note the restart test disproved #15's guess that its `0000b49a` segment was a build/session handle, so that segment is durable but still unexplained. `TallyAdapter` should capture `REMOTEID` at post time | **Still open — one risk left.** Restart-stability **resolved** 2026-09-16: `REMOTEID` byte-identical on all 4 vouchers across a full application close/reopen (`runs/2026-09-16T06-49-*` pre-restart baseline vs `runs/2026-09-16T06-55-*` post-restart), confirming `REMOTEID` as the correlation field on both open questions, per-response and per-restart. **Edit-stability remains untested, and the restart test did nothing to reduce it — no voucher was altered in either run**, so `ALTERID` still equals `MASTERID` everywhere: an absence of a negative result, not a positive one. `post_entry` (#1) may be built on `REMOTEID`; the earlier caveat (don't depend on it in production until the restart case is checked) is now satisfied, leaving amendment-after-posting as the case to settle. Re-run `spikes/p0-02-tally/remoteid_stability_probe.py` either side of a voucher edit in the Tally UI | PENDING |
