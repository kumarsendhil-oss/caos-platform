...
| PENDING:010 | `app/books_connector/tally_adapter.py` — post-verification correlation (BK-07) | Carry-forward, not a new discovery: finding #10's open item (a posted voucher has no link back to its source document) plus finding #11's note that Day Book responses carry `REMOTEID`/`VCHKEY` per `<VOUCHER>`. Finding #14 confirms `VOUCHERNUMBER` cannot serve this role on any company — Tally auto-assigns it (`NUMBERINGSTYLE: Auto Retain`). `TallyAdapter` needs a different field to correlate a posted voucher with its source document for the read-back verification finding #2 makes mandatory; `REMOTEID`/`VCHKEY` captured at post time is the likely answer, not an open design question | Nothing to investigate — resolves when `TallyAdapter.post_entry` is implemented (#1). Confirm `REMOTEID` is stable across a re-read before depending on it | PENDING |
