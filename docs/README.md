...
| `CAOS_Software_Development_Proposal_v0_4.docx` | Customer-facing commercial proposal — scope, timeline, fixed price against milestones, warranty, training, and the licence model. §9's recurring operating costs are priced on the AWS ap-south-1 single-tenant stack per ADR 0007/0009. Earlier versions are in git history, not the working tree — one current copy per deliverable, as with the Feature Documentation. |
