...
| 0003-A1 | Amendment 1: Self-Trigger Loop and Correlating Dropbox Deletions | Accepted | `audit-platform-ADR-0003-amendment-1-webhook-loop-and-deletes.md` |
...
- **0003 is Accepted and now live-verified by P0-07** (2026-09-17). All three previously unobserved specifics — the cursor pattern, the 10-second window, and "App folder" scope behaviour — were confirmed against a real Dropbox app (`spikes/p0-07-dropbox/FINDINGS.md`). The spike also surfaced two gaps the ADR does not cover, resolved in **Amendment 1**. The flag on the ADR is now a record of that verification rather than a pending item.
...
- **0003 Amendment 1 closes a self-trigger loop and a deletion-correlation gap.** P0-07 finding #6 confirmed Dropbox notifies on the app's *own* writes (three of three triggers), and that `deleted` entries carry no `id` or `rev`. Decisions: processed output is written **outside** the watched App folder (a `rev`/`content_hash` ignore-list is the documented fallback, not the default), and the intake component maintains its **own path→ID mapping**. Both are recommendations for Sprint 3; nothing to migrate, since the Document Intake Agent is not written yet.
