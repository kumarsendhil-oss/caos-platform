...
| Item | Status |
|---|---|
| **P0-01** task-sheet timing data | **Not executable — deferred to PM-05 (Sprint 11).** No existing task-sheet data exists to measure; the practice is not tracking timesheets at all, so the item's premise is false rather than blocked. Not an open Phase 0 task and not a remaining blocker — see the sprint plan changelog (v0.2.2) |
| **P0-02** Tally (port 9000 / XML) | **Substantially validated**, live — findings #1–#33, `spikes/p0-02-tally/` |
| **P0-03** shared-inbox routing map | **Core mechanics validated**, live 2026-09-17 — `spikes/p0-03-email-intake/`, findings #1–#8 over IMAP (deliberately not the Gmail API, per ADR 0009/0010's per-deployment provider model). Forwarded mail carries the original sender in no header (#1); two distinct senders shared one domain (#2); dedup key recommendation settled on `(uidvalidity, uid)` with Message-ID as recovery path. Still open: a second provider, IMAP IDLE |
...
| **P0-05** PaddleOCR accuracy (20–50 invoices) | **Harness built and proven; the accuracy validation itself is still blocked** — `spikes/p0-05-paddleocr/`. Extraction and per-field scoring work end to end, with redaction on by default for committed reports. A three-document robustness smoke test on real (non-client) documents gave the first real throughput figure, ~243 s/document on CPU, now in the performance doc §3. **Still needs the 20–50 real client invoices ADR 0008 asks for** — the practice's to provide |
| **P0-06** Zoho Books | **Core mechanics validated**, live — findings #1–#13, `spikes/p0-06-zoho/` |
| **P0-07** Dropbox webhooks | **Core mechanics validated**, live 2026-09-17 — `spikes/p0-07-dropbox/`, 12 evidence records. ADR 0003’s three doc-only claims all confirmed (cursor pattern and that the cursor advances; 10-second window, ack 0.378–1.067 ms; app-relative paths). Finding #6 surfaced two gaps the ADR did not cover — Dropbox notifies on the app’s own writes, and deleted entries carry no id/rev — resolved in **ADR 0003 Amendment 1**. Still unverified: retry/back-off on a slow or failed response |
| IaC tooling decision | **Resolved** 2026-09-17 — ADR 0014 |
| **P0-08** RazorpayX Payroll scoping | **Not started — scoped.** No account, no credential, no live call, no vendor doc read; the test list is questions, not findings — `docs/spikes/p0-08-razorpayx/RUNBOOK.md`. Declared by **ADR 0016** (supersedes ADR 0013 — customer dropped Frappe HR; attendance native in CAOS, exported to RazorpayX; performance quantitative only). **All ten capabilities 0013 delegated to Frappe are unverified against the new vendor.** Gates three things: **leave placement** (T2, decision-critical — the holiday calendar is decided as CAOS's, leave is not), the **TDS agent's 26Q/27EQ scope** (T4, Form 24Q — `PENDING:030`), and **per-deployment isolation** (T7, against ADR 0009). Blocked on obtaining an account, which is a commercial step, not an engineering one — carries a stop rule so it closes with the doc-answerable work done rather than sitting open like P0-04 |

**Phase 0 is now substantially worked through.** P0-02, P0-03, P0-06 and P0-07 have live findings behind them; the IaC question is resolved by ADR 0014. **P0-01 is not among what remains** — it is not executable as scoped and is deferred to PM-05 (Sprint 11), which is a scoping correction rather than an open task. What genuinely remains is **P0-04** (needs a WhiteBooks sandbox account), **P0-05's accuracy validation** (needs 20–50 real client invoices) and, newly, **P0-08** (needs a RazorpayX Payroll account) — all three blocked on things only the practice can supply, none blocked on engineering effort. **P0-08 is the newest and the least worked**: it was scoped on 2026-09-20 alongside ADR 0016 and has produced nothing, so unlike P0-04 it has no first-party-documentation findings to fall back on yet.
...
**Feature Documentation v0.6 shared with the practice, with two open questions
put to them:**
1. Which billing cycles are actually used in practice.
2. That measurement depends on staff marking tasks **started** and **paused** —
   i.e. the data only exists if people do that consistently. This is a
   dependency on behaviour, not on software, and it is the assumption the whole
   measurement story rests on.

Both answers are inputs to the Sprint 1 work; neither is blocking today.

**Four stale cross-references, reported and deliberately not edited.** The
brief scoped this session to Status lines only, and three of these predate it:

- `ADR-0015:5` and `ADR-0016:5` describe **ADR 0013 as *Proposed***; it is
  Superseded by 0016. Pre-existing.
- `ADR-0016:6` describes **ADR 0015 as *Proposed***. This one **was made stale
  by this session's change** and is the first to fix.
- `ADR-INDEX.md:49` says 0013 sits "at the status its own file states
  (*Proposed*)", contradicting its own table row two screens above. Pre-existing.

Body prose in `ADR-0015:528` and `ADR-0016:27` also argues "0013 is still
Proposed". That is a record of the reasoning at the time and is arguably
correct as history; the header cross-references are not.
...
---
*Last updated: 2026-09-21 by `/wrapup` (ADRs 0015 and 0016 Accepted; feature doc v0.6 shared with the practice with two open questions; `PENDING:032` re-prioritised; `ONBOARDING.md` discarded; Sprint 1-2 capacity re-confirmation noted as the remaining pre-sprint item). Earlier the same day, also by `/wrapup`: first run of the revised command — ADRs 0015/0016 written, P0-08 scoped and deferred, feature doc v0.6, PRs #77-#80, the CI trigger removal, and the `/wrapup` change itself. Previous update 2026-09-18 by `/wrapup` (drift audit against actual merged PR state). Previous substantive update 2026-09-16 (`PENDING:009` — sandbox reset resolved as a **validated script**, `reset_sandbox.py`; findings #28–#29, plus the two-company keep-list audit). Originally seeded manually via claude.ai chat. From this point, `/wrapup` should keep this current — if it isn't, that's a sign `/wrapup` isn't being run, not that the file is wrong.*
