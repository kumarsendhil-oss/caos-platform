...
- **DO NOT run `reset_sandbox.py` against `Coastal Services Ltd`** until issue #48 / `PENDING:019` is resolved. Its delete verification cannot distinguish same-numbered vouchers of different types, and this company is now exactly that case.
- **Both Sales probe vouchers are deliberately left uncleaned.** That is a decision, not an oversight — removing them with a script whose verification cannot currently be trusted is the wrong order of operations. Cleanup is follow-up work that belongs with the addressing investigation, under its own authorisation.
- `keep-coastal-services.txt` is now **stale in a way that is not obvious**: its entries `1, 2, 4, 5` are bare numbers and will match the Sales vouchers too. Rebuild it by re-running the voucher-by-voucher audit, never from memory — and not until the addressing question is settled.
